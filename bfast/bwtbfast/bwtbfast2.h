...
typedef struct {
	...
	int32_t alg;
	int32_t seed_len;
	double max_mm_err_rate;
	int32_t max_mm;
	int32_t max_hits;
	int32_t max_seed_hits;
	int32_t seed_ends_only;
} bwtbfast2_thread_t;
#endif
...
void bwtbfast2_core(char *ref_fn, char *read_fn, int32_t compression, int32_t alg, int32_t seed_len, int32_t max_mm, double max_mm_err_rate, int32_t space, int32_t start_read_num, int32_t end_read_num, int32_t max_seed_hits, int32_t max_hits, int32_t seed_ends_only, int32_t num_threads, int32_t queue_length);
#ifdef HAVE_LIBPTHREAD
