...
/* TODO */
int AlignedEndPrint(AlignedEnd *a,
		FILE *outputFP,
		int space,
		int binaryOutput)
{
	...
	if(binaryOutput == TextOutput) {
		...
		for(i=0;i<strlen(a->read);i++) {
			if(fprintf(outputFP, "%d\t",
					(int32_t)a->qual[i]) < 0) {
				return EOF;
			}
		}
...
/* TODO */
int AlignedEndRead(AlignedEnd *a,
		FILE *inputFP,
		int space,
		int binaryInput)
{
...
/* TODO */
int AlignedEndRemoveDuplicates(AlignedEnd **a,
		int length,
		int sortOrder)
{
	int i, prevIndex;
	...
	if(length > 0) {
		/* Sort the array */
		AlignedEndQuickSort(a, 0, length-1, sortOrder, 0, NULL, 0);
		/*
		   AlignedEndMergeSort(a, 0, length-1, sortOrder, 0, NULL, 0);
		   */

		/* Check sort */
		/*
		   for(i=1;i<length;i++) {
		   assert(AlignedEndCompareAtIndex((*a), i-1, (*a), i, sortOrder)<=0);
		   }
		   */
		...
		prevIndex=0;
		for(i=1;i<length;i++) {
			if(AlignedEndCompareAtIndex((*a), prevIndex, (*a), i, sortOrder)==0) {
				/* Do nothing */
			}
			else {
				/* Increment prevIndex */
				...
				AlignedEndCopyAtIndex((*a), i, (*a), prevIndex);
			}
		}
		...
		(*a) = realloc((*a), sizeof(AlignedEnd)*length);
		if(NULL == (*a)) {
			PrintError("AlignedEndRemoveDuplicates",
					"(*a)",
...
void AlignedEndQuickSort(AlignedEnd **a,
		...
		int total)
{
	char *FnName = "AlignedEndQuickSort";
	int i;
	int pivot=-1;
	AlignedEnd *temp=NULL;

	if(low < high) {
		/* Allocate memory for the temp used for swapping */
		temp=malloc(sizeof(AlignedEnd));
		if(NULL == temp) {
			PrintError(FnName,
					"temp",
		...
		AlignedEndInitialize(temp);

		pivot = AlignedEndGetPivot((*a),
				sortOrder,
				low,
				high);
		...
		AlignedEndCopyAtIndex((*a), pivot, temp, 0);
		AlignedEndCopyAtIndex((*a), high, (*a), pivot);
		AlignedEndCopyAtIndex(temp, 0, (*a), high);

		pivot = low;

		for(i=low;i<high;i++) {
			if(AlignedEndCompareAtIndex((*a), i, (*a), high, sortOrder) <= 0) {
				if(i!=pivot) {
					AlignedEndCopyAtIndex((*a), i, temp, 0);
					AlignedEndCopyAtIndex((*a), pivot, (*a), i);
					AlignedEndCopyAtIndex(temp, 0, (*a), pivot);
				}
				pivot++;
			}
		}
		...
		AlignedEndQuickSort(a, low, pivot-1, sortOrder, showPercentComplete, curPercent, total);
		if(showPercentComplete == 1 && VERBOSE >= 0) {
			assert(NULL!=curPercent);
		...
		AlignedEndQuickSort(a, pivot+1, high, sortOrder, showPercentComplete, curPercent, total);
		if(showPercentComplete == 1 && VERBOSE >= 0) {
			assert(NULL!=curPercent);
...
void AlignedEndMergeSort(AlignedEnd **a,
		...
		int total)
{
	char *FnName = "AlignedEndMergeSort";
	int i, ctr;
	int mid = (low + high)/2;
	int startLower =  low;
	int endLower = mid;
	...
	/* Allocate pointers */
	tempEntries = malloc(sizeof(AlignedEnd)*(high-low+1));
	if(NULL == tempEntries) {
		PrintError(FnName,
				"tempEntries",
				"Could not allocate memory",
	...
	for(i=0;i<high-low+1;i++) {
		AlignedEndInitialize(&tempEntries[i]);
	}

	/* Merge the two lists */
	ctr=0;
	while( (startLower <= endLower) && (startUpper <= endUpper)) {
		if(AlignedEndCompareAtIndex((*a), startLower, (*a), startUpper, sortOrder) <= 0) {
			AlignedEndCopyAtIndex((*a), startLower, tempEntries, ctr);
			startLower++;
		}
		else {
			AlignedEndCopyAtIndex((*a), startUpper, tempEntries, ctr);
			startUpper++;
		}
		ctr++;
	}
	...
	for(i=low, ctr=0;
			i<=high;
			i++, ctr++) {
		AlignedEndCopyAtIndex(tempEntries, ctr, (*a), i);
	}

	/* Free memory */
	for(i=0;i<high-low+1;i++) {
		AlignedEndFree(&tempEntries[i]);
	}
	free(tempEntries);
	tempEntries=NULL;
...
/* TODO */
int AlignedEndCompareAtIndex(AlignedEnd *a, int indexA, AlignedEnd *b, int indexB, int sortOrder)
{
	int cmp[5];
	int i;
	int top;

	if(sortOrder == AlignedEndSortByAll) {
		...
		assert(sortOrder == AlignedEndSortByContigPos);
		cmp[0] = (a[indexA].contig <= b[indexB].contig)?((a[indexA].contig<b[indexB].contig)?-1:0):1;
		cmp[1] = (a[indexA].position <= b[indexB].position)?((a[indexA].position<b[indexB].position)?-1:0):1;
...
/* TODO */
void AlignedEndCopyAtIndex(AlignedEnd *src, int srcIndex, AlignedEnd *dest, int destIndex)
{
	if(dest != src || srcIndex != destIndex) {
		AlignedEndCopy(&(src[srcIndex]), &(dest[destIndex]));
	}
}

/* TODO */
void AlignedEndCopy(AlignedEnd *src, AlignedEnd *dest)
{
...
/* TODO */
/* Debugging function */
void AlignedEndCheckReference(AlignedEnd *a, RGBinary *rg, int space)
{
	char *FnName = "AlignedEndCheckReference";
	int i;
	int curPos;
	char rgBase;
		...
		if(reference[i] != GAP) {
			...
			if(rgBase != reference[i]) {
				fprintf(stderr, "\n[%d]\t[%d]\n[%c]\t[%c]\n[%s]\n",
						...
						rgBase,
						reference);
				AlignedEndPrint(a, stderr, space, 0);
				PrintError(FnName,
						NULL,
...
int AlignedEndGetPivot(AlignedEnd *a,
		int sortOrder,
		int low,
		int high) 
{
	...
	cmp[0] = AlignedEndCompareAtIndex(a, low, a, pivot, sortOrder); 
	cmp[1] = AlignedEndCompareAtIndex(a, low, a, high, sortOrder); 
	cmp[2] = AlignedEndCompareAtIndex(a, pivot, a, high, sortOrder); 
...
int64_t AlignedEndGetSize(AlignedEnd *a)
{
	int64_t size = 0;

	size += sizeof(AlignedEnd);
